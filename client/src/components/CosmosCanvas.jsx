...
const Avatar = ({ user, isCurrentUser, onClick, interpolatedPosition }) => {
  ...
  const displayPos = isCurrentUser
    ? user.position
    : interpolatedPosition || user.position;
  ...
  useEffect(() => {
    ...
    const gameLoop = () => {
      ...
      if (dx !== 0 || dy !== 0) {
        const newX = Math.max(
          50,
          Math.min(CANVAS_WIDTH - 50, user.position.x + dx),
        );
        const newY = Math.max(
          50,
          Math.min(CANVAS_HEIGHT - 50, user.position.y + dy),
        );
      ...
      if (containerRef.current) {
        ...
        const clampedTargetX = Math.max(
          0,
          Math.min(CANVAS_WIDTH - containerWidth, targetCamX),
        );
        const clampedTargetY = Math.max(
          0,
          Math.min(CANVAS_HEIGHT - containerHeight, targetCamY),
        );

        // Smooth camera interpolation
        cameraRef.current.x +=
          (clampedTargetX - cameraRef.current.x) * CAMERA_LERP;
        cameraRef.current.y +=
          (clampedTargetY - cameraRef.current.y) * CAMERA_LERP;
  ...
  return (
    ...
    >
        ...
        {users.map((otherUser) => (
          <Avatar
            ...
            interpolatedPosition={
              interpolatedPositions.current[otherUser.odestined]
            }
          />
        ))}
      ...
      <div className="absolute bottom-4 left-4 glass px-4 py-2 rounded-lg text-sm text-gray-400">
        <kbd className="px-1.5 py-0.5 bg-violet-600/30 rounded text-violet-300">
          Arrow Keys
        </kbd>{" "}
        or{" "}
        <kbd className="px-1.5 py-0.5 bg-violet-600/30 rounded text-violet-300">
